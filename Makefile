...
# mailcow-watchdog, apart from the service-specific targets at the very bottom.
# Change it in one repository, then copy it to the other — see CONVENTIONS.md.

BINARY   := dockerapi
CMD_PKG  := ./cmd/dockerapi/
...
test-integration: ## Run the tests that need a real Docker daemon.
	CGO_ENABLED=1 $(GO) test -tags=integration ./... -race -count=1

.PHONY: fuzz
fuzz: ## Fuzz the shell quoting and the stream demuxing (30s per target).
...
.PHONY: compare
compare: ## Compare the responses against the Python implementation (needs Docker).
